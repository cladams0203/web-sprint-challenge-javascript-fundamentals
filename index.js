...
function lowerCaseNames(a) {
  return a.map((x) => x.animal_name.toLowerCase());
}
...
function lowPopulationAnimals(a) {
  return a.filter((x) => x.population < 5);
}
...
function USApop(a) {
  return a.reduce((t, x) => t + x.population, 0);
}
